...
-- Instead of loading the whole, huge database into Heroku (> 7 million lines) the idea is only to load
-- in the query results (13k lines)
...
CREATE TABLE bikeid_stats AS
  SELECT
   ...
   (SELECT COUNT (startstationname) FROM citibike_rides WHERE bikeid = $1 GROUP BY startstationname ORDER BY COUNT DESC LIMIT 1);
